...
class HebbianMetaLearningRobot(object):
    """
    A meta-learning robot that combines MAML with Hebbian learning to quickly
    adapt to new maze environments and learn from experience.
    """
    
    def __init__(self, maze_dim):
        '''
        Initialize the Hebbian meta-learning robot
        '''
        ...
        self.hebbian_decay = 0.99  # Decay factor for Hebbian weights
        self.hebbian_threshold = 0.1  # Threshold for Hebbian updates
        ...
        # Hebbian weight matrix for rapid adaptation
        self.hebbian_weights = np.random.randn(self.input_size, self.hidden_size) * 0.01
        ...
        self.few_shot_memory = deque(maxlen=100)  # Store recent successful experiences
        self.few_shot_threshold = 0.8  # Confidence threshold for few-shot decisions
    ...
    def _forward_pass(self, state, weights=None, use_hebbian=True):
        """Forward pass through the neural network with optional Hebbian enhancement"""
        if weights is None:
        ...
        # Apply Hebbian enhancement if enabled
        if use_hebbian:
            hebbian_enhancement = np.dot(state, self.hebbian_weights)
            hidden += hebbian_enhancement * 0.1  # Small contribution from Hebbian learning
    ...
    def _hebbian_update(self, input_activity, output_activity):
        """Update Hebbian weights based on input-output correlation"""
        # Hebbian rule: Δw = η * input * output
        delta_w = self.hebbian_lr * np.outer(input_activity.flatten(), output_activity.flatten())
        
        # Apply threshold to prevent excessive weight growth
        delta_w = np.where(np.abs(delta_w) > self.hebbian_threshold, 
                          np.sign(delta_w) * self.hebbian_threshold, delta_w)
        
        # Update Hebbian weights
        self.hebbian_weights += delta_w
        
        # Apply weight decay
        self.hebbian_weights *= self.hebbian_decay
    ...
    def _select_action(self, state, training=True):
        """Select action using epsilon-greedy policy with few-shot learning"""
        if training and random.random() < self.epsilon:
            return random.randint(0, 3)
        
        # Check few-shot memory for similar situations
        if len(self.few_shot_memory) > 0:
            few_shot_action = self._few_shot_decision(state)
            if few_shot_action is not None:
                return few_shot_action
        
        # Get Q-values from neural network
        q_values, _ = self._forward_pass(state)
        return np.argmax(q_values)
    
    def _few_shot_decision(self, current_state):
        """Make decision based on few-shot learning from similar experiences"""
        best_similarity = 0
        best_action = None
        
        for memory_item in self.few_shot_memory:
            memory_state = memory_item['state']
            memory_action = memory_item['action']
            memory_reward = memory_item['reward']
            
            # Compute similarity (cosine similarity)
            similarity = np.dot(current_state.flatten(), memory_state.flatten()) / \
                       (np.linalg.norm(current_state) * np.linalg.norm(memory_state))
            
            # Only consider positive rewards and high similarity
            if similarity > self.few_shot_threshold and memory_reward > 0:
                if similarity > best_similarity:
                    best_similarity = similarity
                    best_action = memory_action
    ...
    def _meta_update(self, task_experiences):
        """Perform meta-learning update using MAML with Hebbian enhancement"""
    ...
    def _update_few_shot_memory(self, state, action, reward):
        """Update few-shot learning memory with successful experiences"""
        if reward > 0:  # Only store positive experiences
            ...
            self.few_shot_memory.append(memory_item)
    ...
    def next_move(self, sensors):
        """
        Determine next move using Hebbian meta-learning approach
        """
        # Update maze map
        ...
        if self.run == 0:  # Only during training
            ...
            # Update few-shot memory
            self._update_few_shot_memory(current_state, action_idx, reward)
            
            # Perform Hebbian update
            _, hidden_activity = self._forward_pass(current_state)
            self._hebbian_update(current_state, hidden_activity)
    ...
    def get_meta_learning_stats(self):
        ...
        return {
            ...
            'few_shot_memory_size': len(self.few_shot_memory),
            'hebbian_weights_norm': np.linalg.norm(self.hebbian_weights)
        }
    ...
    def save_model(self, filename):
        ...
        model_data = {
            ...
            'few_shot_memory': list(self.few_shot_memory)
        }
    ...
    def load_model(self, filename):
        ...
        self.meta_weights = model_data['meta_weights']
        self.hebbian_weights = model_data['hebbian_weights']
        self.few_shot_memory = deque(model_data['few_shot_memory'], maxlen=100)
